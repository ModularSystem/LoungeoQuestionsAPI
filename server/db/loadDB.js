...
const importQuestions = () => {
  let stream = fs.createReadStream('csv/questionsCSV.csv')
  let csvData = [];
...
async function importQuestion(values) {
  ...
  try{
    const questionReturn = await pool.query(`
      INSERT INTO questions(question_id,question_body,question_date,asker_name,user_id,question_helpfulness, reported, product_id)
      VALUES (${id}, '${body}', (to_timestamp(${date}/ 1000.0)), '${name}',${userId}, ${helpful}, ${reported}, ${productId})
    `)
